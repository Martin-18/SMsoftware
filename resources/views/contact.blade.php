...
<div class="container mx-auto px-6 py-16 max-w-2xl">
    ...
    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-slate-900 mb-4">Contáctanos</h1>
        <p class="text-slate-500 text-lg">Cuéntanos tu proyecto y te respondemos en menos de 24 horas.</p>
    </div>
        ...
        <form action="{{ route('contact.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}"
                       ...
                       class="w-full px-4 py-3 rounded-lg border {{ $errors->has('name') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} focus:border-blue-500 outline-none transition">
                @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}"
                           ...
                           class="w-full px-4 py-3 rounded-lg border {{ $errors->has('email') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} focus:border-blue-500 outline-none transition">
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <select name="service_interest"
                        class="w-full px-4 py-3 rounded-lg border {{ $errors->has('service_interest') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} focus:border-blue-500 outline-none transition">
            ...
            <div>
                ...
                <textarea name="message" rows="5"
                          ...
                          class="w-full px-4 py-3 rounded-lg border {{ $errors->has('message') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} focus:border-blue-500 outline-none transition resize-none">{{ old('message') }}</textarea>
                @error('message')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
